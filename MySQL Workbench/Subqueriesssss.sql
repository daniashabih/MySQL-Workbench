...
insert into e_retired(select e_id, e_name, e_position, e_salary from employee where d_id = (select d_id from department where d_name = "sales"));

select e_name from employee where d_id = (select d_id from department where d_location = "texas");
...
select * from employee where d_id not in (select d_id from department where d_ranking = 1 or d_ranking = 2 or d_ranking = 3);
